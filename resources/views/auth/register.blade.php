    ...
    <h1 class="text-brown-default text-2xl md:text-3xl mb-5">Register</h1>
        <form method="POST" action="{{ route('register') }}">
            ...
            <div class="flex flex-col items-center justify-center mb-2">
                ...
                <input id="name" type="text" class="rounded border-2 border-brown-default outline-none py-1 px-3 text-grey-darkest @error('name') is-invalid @enderror" name="name" value="{{ old('name') }}" required autocomplete="name" autofocus>
                    @error('name')
            ...
            <div class="flex flex-col items-center justify-center mb-2">
                ...
                <input id="email" type="email" class="rounded border-2 border-brown-default outline-none py-1 px-3 text-grey-darkest @error('name') is-invalid @enderror" name="email" value="{{ old('email') }}" required autocomplete="email">
                    @error('email')
                    <div class="text-red-600">
                        <p>{{$message}}</p>
                    </div>
            ...
            <div class="flex flex-col items-center justify-center mb-2">
                <label for="password" class="text-brown-dark text-lg mb-2 mr-5 uppercase font-bold text-grey-darkest">{{ __('Password') }}</label>
                    <input id="password" type="password" class="rounded border-2 border-brown-default outline-none py-1 px-3 text-grey-darkest @error('password') is-invalid @enderror" name="password" required autocomplete="new-password">
                    @error('password')
                    <div class="text-red-600">
                        <p>{{$message}}</p>
                    </div>
